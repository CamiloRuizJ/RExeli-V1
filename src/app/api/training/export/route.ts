...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if minimum examples are met
    if (!stats.meets_minimum) {
      return NextResponse.json<ApiResponse>({
        success: false,
        error: `Insufficient training data. Have ${stats.total_verified} verified documents, need at least 50 for OpenAI fine-tuning.`
      }, { status: 400 });
    }
