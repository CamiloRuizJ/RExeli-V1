-- =====================================================
-- Migration 016: Allow Trigger to Bypass RLS
-- =====================================================
-- Problem: Trigger runs without authenticated user context, so auth.uid() = NULL
-- Solution: Make trigger function bypass RLS when updating existing users
-- =====================================================

-- Update trigger function to explicitly bypass RLS for admin linking
...
BEGIN
  ...
  -- Check if user already exists (bypassing RLS)
  ...
  IF existing_user_id IS NOT NULL THEN
    ...
    -- Use a direct UPDATE that bypasses RLS policies
    EXECUTE format(
      'UPDATE public.users SET auth_user_id = %L, email_verified = %L, provider = %L, updated_at = NOW() WHERE id = %L',
      NEW.id,
...
SELECT '✅ Trigger function updated to bypass RLS - admin creation will now work!' as status;
